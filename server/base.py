...
class BaseProtocol:
    ...
    async def run(self):
        ...
        self.host_id, self.identifier = util.get_identifier(self.ip, self.port)
        print(f"Running server on {self.ip}:{self.port}")
        print(f"Server identifier is {self.identifier}")
